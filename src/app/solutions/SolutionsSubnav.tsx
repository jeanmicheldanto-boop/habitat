          ...
          {SECTIONS.map((s, idx) => {
            ...
            return (
              <React.Fragment key={s.id}>
                <li style={{ width: "100%" }}>
                  <a href={`#${s.id}`} onClick={goTo(s.id)} style={{ display: "block", width: "100%", padding: "0.9rem 0.5rem", background: isActive ? "#ececec" : "#f6f6f6", color: isActive ? "#a85b2b" : "#444", fontWeight: isActive ? 600 : 400, fontSize: "1.05rem", textAlign: "center", textDecoration: "none", cursor: "pointer" }}>
                    {s.label}
                  </a>
                </li>
