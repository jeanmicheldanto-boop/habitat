/**
 * Script pour envoyer la campagne via Elastic Email
 * Usage: node send-campaign-elastic.js
 * 
...
if (!ELASTICEMAIL_API_KEY || ELASTICEMAIL_API_KEY === 'votre-clé-elastic-email-ici') {
  ...
  console.error('   ELASTICEMAIL_API_KEY=votre-clé-ici');
  console.error('');
  console.error('💡 Obtenez votre clé :');
  console.error('   https://elasticemail.com/ → Settings → API → Create API Key');
  process.exit(1);
}
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          <tr>
            <td style="padding: 40px 40px 30px;">
              <p style="font-size: 16px; color: #333; margin: 0;">Madame, Monsieur,</p>
            </td>
          </tr>
          <tr>
...
async function sendEmailViaElastic(email, subject, html) {
  ...
  if (!result.success) {
    throw new Error(`Elastic Email error: ${result.error || JSON.stringify(result)}`);
  }
...
async function sendCampaign() {
  console.log('\n📧 CAMPAGNE RÉFÉRENCEMENT - Habitat Intermédiaire (via Elastic Email)\n');
  ...
  try {
    ...
    for (let i = 0; i < etabs.length; i++) {
      ...
      try {
        const html = getEmailTemplate(etab.nom);
        const result = await sendEmailViaElastic(
          etab.email,
          CAMPAIGN_CONFIG.subject,
          html
        );
        
        console.log(`   ✅ Envoyé - TransactionID: ${result.data.transactionid}`);
        totalSent++;
...
if (!CAMPAIGN_CONFIG.testMode) {
  ...
  console.log(`   ${2016} emails seront envoyés`);
